...
router.get('/', (req, res)=>{
  //为了获得所有菜品，必须先查询菜品类别
  pool.query('SELECT cid,cname FROM HotPot_category ORDER BY cid', (err, result)=>{
    if(err)throw err;
    ...
    for(let c of categoryList){
      pool.query('SELECT * FROM HotPot_dish WHERE categoryId=? ORDER BY did DESC', c.cid, (err, result)=>{
        if(err)throw err;
        c.dishList = result;
        finishCount++;
...
*/
router.post('/', (req, res)=>{
  pool.query('INSERT INTO HotPot_dish SET ?', req.body, (err, result)=>{
    if(err)throw err;
